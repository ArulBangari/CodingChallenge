...
class ShoppingDataAnalyzer:
    ...
    def most_used_payment_method(self):
        ...
        payment_count = self.df["payment_method"].value_counts()
        # idmax() returns the index of the maximum value
        most_used_payment = payment_count.idxmax()
        # Get the frequency of the most used payment
        most_used_payment_freq = payment_count[most_used_payment]
        return [most_used_payment, most_used_payment_freq]
    ...
    def day_with_most_sales(self):
        ...
        # .loc[row_index, column] allows you to select based on a row index and column variables
        # self.df["sales"].idxmax() returns the id of the row with the max sales
        # pretty much choosing the row and column and then getting the value
        date_with_most_sales = self.df.loc[self.df["sales"].idxmax(), 'invoice_date']
        max_sales_count = self.df["sales"].max()
        return [date_with_most_sales, max_sales_count]